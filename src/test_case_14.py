...
def launch_training(line="12_13_14", interaction_frequency=10000, reuse=False, writer_logdir=None):

    if writer_logdir is None:
        writer_logdir = f"../results/test/case_14/interaction_freq_{interaction_frequency}/{time.time()}"

    if not isinstance(reuse, bool):
        reuse = reuse == True

    # model definition
    params = json.load(open(f"params_case_14_deco_line_{line}.json", 'rb'))
    ...
    params['env_kwargs']["eval_params_path"] = os.path.join(params['folder_path'],
                                                            params['env_kwargs']["eval_params_path"])

    params['env_kwargs']["use_np"] = False
    ...
    model = PreferenceReinforceGUI(env_class=BatchSymbolicRegressionEnv,
                                   ...
                                   user=SelectRandomRewardUser(**user_params),
